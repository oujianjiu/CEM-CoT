...
from CBL_nn import CBL_nn, Roberta_CBL, GPT_CBL
...
if __name__ == "__main__":
    ...
    if 'roberta' in backbone:
        if 'no_backbone' in cbl_name:
            ...
            cbl = CBL_nn(len(concept_set), args.dropout).to(device)
            ...
            backbone_cbl = Roberta_CBL(len(concept_set), args.dropout).to(device)
        ...
        if 'no_backbone' in cbl_name:
            ...
            backbone_cbl = GPT_CBL(len(concept_set), args.dropout).to(device)
            backbone_cbl.load_state_dict(torch.load(args.cbl_path, map_location=device))
            backbone_cbl.eval()
    else:
        raise Exception("backbone should be roberta or gpt2")
