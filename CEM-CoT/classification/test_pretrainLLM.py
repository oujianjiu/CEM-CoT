...
from CBL_nn import blackbox_baseline, gpt_baseline, MLP
...
if __name__ == "__main__":
    ...
    if 'roberta' in backbone:
        if args.tune_mlp_only:
            ...
            LM = blackbox_baseline(CFG.class_num[args.dataset], args.projection_dim, args.dropout).to(device)
        ...
        if args.tune_mlp_only:
            ...
            print("loading gpt2...")
            LM = gpt_baseline(CFG.class_num[args.dataset], args.projection_dim, args.dropout).to(device)
            LM.load_state_dict(torch.load(args.model_path, map_location=device))
            LM.eval()
    else:
        raise Exception("backbone should be roberta or gpt2")
